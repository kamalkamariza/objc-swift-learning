...
//            [device setExposureModeCustomWithDuration:CMTimeMakeWithSeconds( 0.02, 1000*1000*1000 ) ISO:300.0
...
- (void)turnTorchOn:(BOOL)on withValue:(float)value
{
    ...
    if (captureDeviceClass != nil) {
        ...
        if ([device hasTorch] && [device hasFlash]){
            ...
            [device lockForConfiguration:nil];
            if (on) {
                [device setTorchModeOnWithLevel:value error:NULL];
...
//        [self processAudioSampleBuffer:sampleBuffer];
    ...
    else
    {
        ...
        runAsynchronouslyOnVideoProcessingQueue(^{
            ...
            if (self.delegate)
            {
//                NSLog(@"DEBUG Getting output from camera");
